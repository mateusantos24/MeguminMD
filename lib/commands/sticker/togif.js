...
try {
    ...
    if (ffmpegloc?.path) ffmpegBin = ffmpegloc.path;
} catch {
    // No Android/Termux usamos o ffmpeg do sistema.
}
...
async function extractEmbeddedImageFromLottieBuffer() {
    throw new Error('Conversao de figurinha Lottie para video nao está disponivel nesta build.');
}

...
function detectRuntimeEnvironment() {
    ...
    const isUnixLikeLinux = isLinux || isAndroidPlatform;
    const isTermux = isUnixLikeLinux && detectTermux(env);
    const isAndroid = isAndroidPlatform || (isUnixLikeLinux && detectAndroid(env, isTermux));
    const androidInfo = detectAndroidDeviceInfo(isAndroid);
    const containerType = isUnixLikeLinux ? detectContainer() : '';
    const virtualization = isUnixLikeLinux ? detectVirtualization() : '';
